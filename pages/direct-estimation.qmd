...
| Unknown Domain Sizes; only analysis weights | Hájek estimator |$\hat{\theta}_d^{DIR}=\frac{\sum_{i \in s_d}w_{di}y_{di}}{\sum_{i \in s_d}w^A_{di}}$| Can only be used for estimating *means or proportions* when only using analysis weights and no reliable domain size is available |
...
The Horvitz-Thompson estimates are generally more precise, with median CVs below 100%, because they use known domain sizes alongside grossing weights. By contrast, the Hájek estimate for `commute_distance` relies only on analysis weights and has much higher relative uncertainty. This illustrates why suitable domain-size information is valuable in SAE.

Another remedy is to redefine the estimand associated with `commute_distance` so that it applies to a population with known domain sizes, such as all adults, rather than a subgroup whose MSOA totals may be unavailable. However, this would essentially change what is being measured.
