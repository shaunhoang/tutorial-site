...
## Select preliminary covariates
SAE improves precision by "borrowing strength" from auxiliary variables (i.e., covariates). This is critical for domains with sparse or non-existent survey data. For area-level models, these covariates must be aggregated for every target domain. For unit-level modesl, these covariates must exist at the population unit level.

Aim for **under 10 preliminary covariates per indicator**. While formal filtering happens later, limiting the initial pool reduces the risk of overfitting and multicollinearity. Some selection rules of thumb are:
...
- **Alignment:** Ensure definitions are identical across both the survey and the population frame (e.g., the exact age range defining an "adult").
- **Temporal Stability:** Opt for variables that remain stable over time. Because population frames (like the Census) often lag behind current survey data, stable covariates minimise errors caused by this time gap.
