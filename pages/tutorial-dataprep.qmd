...
|3| Rate | Walking as main transport at least weekly per 1,000 residents | All residents | MSOAs in Wales |
...
|3| `freq_walker` | [`AtFrqWlk10`] Binary: `1` if walking for transport at least weekly | Rate (per 1k)| MSOA | `SampleTravelWeight`, analysis weight; to be scaled to grossing weight  |
...
- `SampleTravelWeight`: an analysis weight for the active-travel module. It is exported for `freq_walker`, then scaled to a Wales-wide grossing weight in the direct-estimation workbook.
...
survey_indicators <- nsw_raw |>
  mutate(
    commute_distance_raw = clean_nsw(TravWkDist),
    bus_used_12m = clean_nsw(Bus12M),
    bus_satisfaction = clean_nsw(BusOverSat),
    walk_frequency = clean_nsw(AtFrqWlk10)
  ) |>
  transmute(
    unit_id,
    commute_distance = as.numeric(commute_distance_raw),

    bus_nosat = case_when(
      bus_used_12m != 1 ~ NA_real_,
      bus_satisfaction %in% c(4, 5) ~ 1,
      !is.na(bus_satisfaction) ~ 0,
      TRUE ~ NA_real_
    ),

    freq_walker = case_when(
      is.na(walk_frequency) ~ NA_real_,
      walk_frequency %in% 1:3 ~ 1,
      TRUE ~ 0
    )
  )
...
indicator_area_summary <- indicator_observations |>
  ...
  summarise(
    ...
    areas_no_data = areas_total - n_distinct(domain_id),
    areas_small_samp = areas_no_data + sum(non_missing <= 2),
    .groups = "drop"
  )

indicator_area_summary
```
This summary highlights areas with no or few data (`n <= 2`). For two indicators, `bus_nosat` and `commute_distance`, the number of small-sample areas is below 10, which is not a major concern at this stage. `freq_walker` has many more small-sample areas.
...
- `ind_weight_freq_walker.csv`, with active-travel module analysis weight, which will be scaled to a Wales-wide grossing weight during direct estimation.
